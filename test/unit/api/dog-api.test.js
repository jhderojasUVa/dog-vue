...
describe("dog-api", () => {
  ...
  it('SendData is ok', async () => {
    const goodData = {
      fileUrl: 'something',
      description: 'foo'
    }

    const response = await API.sendThings(goodData);
    expect(response).toEqual(true);
  })
  it('SendData is KO', async () => {
    const badData = {
      description: 'foo'
    }

    try {
      const response = await API.sendThings(badData);
    } catch (err) {
      expect(err).toEqual('Bad boy');
    }
  });
});
